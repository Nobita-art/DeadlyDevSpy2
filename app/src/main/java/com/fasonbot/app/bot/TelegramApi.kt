...
object TelegramApi {
    ...
    private fun getApiUrl(): String = "$BASE_URL${BotConfig.getBotToken()}"
    ...
    fun deleteMessage(context: Context, messageId: Int?) {
        if (messageId == null) return
        try {
            val url = "${getApiUrl()}/deleteMessage?chat_id=${getChatId(context)}&message_id=$messageId"
    ...
    fun answerCallbackQuery(context: Context, callbackQueryId: String, text: String? = null) {
        try {
            ...
            val request = Request.Builder()
                .url("${getApiUrl()}/answerCallbackQuery")
                .post(json.toString().toRequestBody("application/json".toMediaType()))
                .build()
    ...
    fun sendPhoto(context: Context, photoFile: File, caption: String? = null) {
        try {
            ...
            client.newCall(request).execute().use { response ->
                if (!response.isSuccessful) {
                    Log.e(TAG, "sendPhoto failed: HTTP ${response.code}")
                }
            }
        } catch (e: Exception) {
