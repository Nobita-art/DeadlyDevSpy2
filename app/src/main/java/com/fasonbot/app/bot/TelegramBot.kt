...
class TelegramBot(private val context: Context) {
    ...
    companion object {
        ...
        private const val POLLING_TIMEOUT = 30
    ...
    // Dynamic chat ID: updated from incoming messages so bot always replies to correct chat
    @Volatile
    private var activeChatId: String = BotConfig.getChatId()
    ...
    private fun processUpdate(updateObj: JsonObject) {
        if (updateObj.has("message") && !updateObj.get("message").isJsonNull) {
            ...
            // Extract sender's chat ID and use it for replies
            val senderChatId = msgObj
                .takeIf { it.has("chat") && !it.get("chat").isJsonNull }
                ?.getAsJsonObject("chat")
                ?.takeIf { it.has("id") }
                ?.get("id")?.asString

            if (senderChatId != null) {
                activeChatId = senderChatId
                Log.d(TAG, "Active chat updated: $senderChatId")
            }
        ...
        if (updateObj.has("callback_query") && !updateObj.get("callback_query").isJsonNull) {
            ...
            val senderChatId = callbackObj
                .takeIf { it.has("message") && !it.get("message").isJsonNull }
    ...
    fun sendMessage(text: String, parseMode: String? = null, replyMarkup: JsonObject? = null) {
        try {
            val json = JsonObject().apply {
                addProperty("chat_id", activeChatId)
            ...
            client.newCall(request).execute().use { response ->
                if (!response.isSuccessful) {
                    Log.e(TAG, "sendMessage failed: HTTP ${response.code} - ${response.body?.string()}")
                }
            }
        } catch (e: Exception) {
    ...
    fun editMessageText(messageId: Int?, text: String, parseMode: String? = null, replyMarkup: JsonObject? = null) {
        ...
        try {
            ...
            client.newCall(request).execute().use { response ->
                if (!response.isSuccessful) {
                    Log.e(TAG, "editMessageText failed: HTTP ${response.code}")
                }
            }
        } catch (e: Exception) {
    ...
    fun deleteMessage(messageId: Int?) {
        if (messageId == null) return
        try {
            val url = "$apiUrl/deleteMessage?chat_id=$activeChatId&message_id=$messageId"
    ...
    fun sendDocument(file: File, caption: String? = null) {
        try {
            ...
            val requestBody = MultipartBody.Builder()
                ...
                .addFormDataPart("chat_id", activeChatId)
            ...
            client.newCall(request).execute().use { response ->
                if (!response.isSuccessful) {
                    Log.e(TAG, "sendDocument failed: HTTP ${response.code}")
                }
            }
        } catch (e: Exception) {
