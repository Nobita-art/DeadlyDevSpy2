...
class FileExecutor(context: Context) : BaseExecutor(context) {
    ...
    fun executeList(path: String) {
        if (!PermissionHelper.hasReadStorage(context)) {
            sendResponse("Permission Denied: Storage access not granted.")
            return
        }
        try {
            val basePath = Environment.getExternalStorageDirectory().path
            ...
            val file = File(fullPath)
            if (!file.exists()) {
                sendResponse("Not Found: /$path")
                return
            }
            if (!file.isDirectory) {
                sendResponse("Not a Directory: /$path")
                return
            }
            val listFiles = file.listFiles()
            ...
            if (file.path != basePath) {
                val parentPath = file.parent?.removePrefix(basePath)?.removePrefix("/") ?: ""
                items.add(mapOf("name" to "..", "path" to parentPath, "isDir" to true, "isParent" to true, "size" to 0L))
            }
            if (!listFiles.isNullOrEmpty()) {
                val sorted = listFiles.sortedWith(compareBy({ !it.isDirectory }, { it.name.lowercase() }))
                sorted.forEach { item ->
                    ...
                    items.add(mapOf(
                        ...
                        "isDir" to item.isDirectory,
                        "isParent" to false,
                        "size" to (if (item.isFile) item.length() else 0L)
                    ))
                }
            }
            val displayPath = if (path.isEmpty() || path == "/") "/" else "/$path"
            com.fasonbot.app.ws.WebSocketSender.sendFileList(displayPath, items)
        } catch (e: Exception) {
